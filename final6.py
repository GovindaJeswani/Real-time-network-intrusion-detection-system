...
CONFIG = {
    ...
    'fast_model': RandomForestClassifier(n_estimators=10, max_depth=5),
    'slow_model': LGBMClassifier(n_estimators=100),
    'queue_size': 2000,
    'num_workers': 4,
    'packet_limit': 500,
...
def calculate_confidence(predictions):
    return np.max(predictions) if hasattr(predictions, '__iter__') else 1.0

# Process a packet using the fast and slow models
def process_packet(packet, fast_model, slow_model, stats):
    features = packet['features']
    fast_prediction = fast_model.predict([features])[0]
    fast_confidence = calculate_confidence(fast_model.predict_proba([features])[0])
    
    with stats['lock']:
        stats['fast_model_count'] += 1
        stats['fast_confidences'].append(fast_confidence)

    if fast_confidence < CONFIG['confidence_threshold']:
        with stats['lock']:
            stats['slow_model_count'] += 1
        slow_prediction = slow_model.predict([features])[0]
        return slow_prediction
    
    return fast_prediction

# Worker function for processing packets
def worker(input_queue, output_queue, fast_model, slow_model, stats):
    while True:
        packet = input_queue.get()
        if packet is None:
            break
        start_time = time.time()
        prediction = process_packet(packet, fast_model, slow_model, stats)
...
def calculate_metrics(stats, packets, predicted_labels, true_labels):
    ...
    avg_fast_confidence = sum(stats['fast_confidences']) / len(stats['fast_confidences']) if stats['fast_confidences'] else 0
    ...
    print(f"Packets Processed by Fast Model: {stats['fast_model_count']}")
    print(f"Packets Processed by Slow Model: {stats['slow_model_count']}")
    print(f"Average Confidence of Fast Model: {avg_fast_confidence:.2f}")
    print(f"Confidence Threshold for Escalation: {CONFIG['confidence_threshold']}")
...
def main():
    ...
    train_path = r"Y:\BITS-Life\Sem-2\NS\Project\NTA_Basic\training.csv"
    test_path = r"Y:\BITS-Life\Sem-2\NS\Project\NTA_Basic\testing.csv"
    
    # Load and preprocess the dataset
    X_train, y_train, X_test, y_test, scaler = load_and_preprocess_data(train_path, test_path)
    
    # Train the models on the first 7 features
    X_train_7 = X_train[:, :7]  # Use only the first 7 features
    X_test_7 = X_test[:, :7]
    
    fast_model = CONFIG['fast_model']
    slow_model = CONFIG['slow_model']
    fast_model.fit(X_train_7, y_train)
    slow_model.fit(X_train_7, y_train)
    fast_only = CONFIG['fast_model']
    slow_only = CONFIG['slow_model']
    fast_only.fit(X_train_7, y_train)
    slow_only.fit(X_train_7, y_train)
    
    # Evaluate the models on the testing data
    print("\nEvaluating Fast Model on Testing Data:")
    y_test_pred_fast = fast_model.predict(X_test_7)
    f1_fast, accuracy_fast = evaluate_predictions(y_test, y_test_pred_fast)
    print(f"Fast Model - F1 Score: {f1_fast:.4f}, Accuracy: {accuracy_fast:.4f}")
    ...
    workers = [Thread(target=worker, args=(input_queue, output_queue, fast_model, slow_model, stats)) for _ in range(CONFIG['num_workers'])]
    for t in workers:
        t.start()
    ...
    avg_latency = sum(stats['latencies']) / len(stats['latencies']) if stats['latencies'] else 0
    packets_per_second = total_packets / (stats['end_time'] - stats['start_time']) if stats['end_time'] > stats['start_time'] else 0
    avg_confidence = sum(stats['fast_confidences']) / len(stats['fast_confidences']) if stats['fast_confidences'] else 0
    three_pkt_flow_miss_rate = calculate_flow_miss_rate(packets)
    f1, accuracy = evaluate_predictions(true_labels, predicted_labels)
    ...
    return {
        ...
        'fast_model_count': stats['fast_model_count'],
        'f1_fast': f1_fast,
        'accuracy_fast': accuracy_fast,
        'slow_model_count': stats['slow_model_count'],
        'f1_slow': f1_slow,
        'accuracy_slow': accuracy_slow,
