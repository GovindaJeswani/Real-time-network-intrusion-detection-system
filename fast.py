...
CONFIG = {
    ...
    'fast_model': RandomForestClassifier(n_estimators=10, max_depth=5),  # Only fast model is used
    'queue_size': 2000,
    'num_workers': 4,
    'packet_limit': 500,
    'confidence_threshold': 0.6  # Not used in single-model architecture
}
...
# Process a packet using the fast model
def process_packet(packet, fast_model, stats):
    features = packet['features']
    prediction = fast_model.predict([features])[0]
    confidence = np.max(fast_model.predict_proba([features])[0])
...
def worker(input_queue, output_queue, fast_model, stats):
    while True:
        packet = input_queue.get()
        if packet is None:
            break
        start_time = time.time()
        prediction = process_packet(packet, fast_model, stats)
...
def calculate_metrics(stats, packets, predicted_labels, true_labels):
    ...
    print(f"Average Confidence of Fast Model: {avg_confidence:.2f}")
...
def main():
    ...
    workers = [Thread(target=worker, args=(input_queue, output_queue, fast_model, stats)) for _ in range(CONFIG['num_workers'])]
    for t in workers:
        t.start()
    ...
    return {
        ...
        'slow_model_count': 0,  # Fast model doesn't use slow model
